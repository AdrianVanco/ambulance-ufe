...
@Component({
  tag: 'xvancoa-ambulance-wl-editor',
  styleUrl: 'xvancoa-ambulance-wl-editor.css',
  shadow: true,
})
export class XvancoaAmbulanceWlEditor {
  @Prop() entryId: string;
  ...
  private async assumedEntryDateAsync(): Promise<Date> {
    try {
      ...
      return new Date(Math.max(Date.now(), lastPatientOut)); // min to max fix
    } catch (err: any) {
      return new Date();
    }
  }
